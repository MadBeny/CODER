...
class RichUI:
    ...
    def node_context(self, name: str, is_error: bool = False):
        ...
        with Live(Text("", style="subtle"), console=self.console, refresh_per_second=20) as live:
            def animate():
                while state.running:
                    if state.message:
                        ...
                        display_line.append(f" {spinner}", style="#999999")
            ...
            try:
                ...
                if not is_error:
                    success_text = Text(f"[{name.upper()}] ⸻ Task Complete", style="#111111")
    ...
    def log_thought(self, text: str):
        ...
        panel = Panel(
            md,
            title="[dim]COGNITIVE PROCESS[/dim]",
            border_style="dim",
            box=box.ROUNDED,
            padding=(1, 2),
        )
    ...
    def log_final(self, text: str):
        """Renders the final synthesis with high-impact typography."""
        if not text or not text.strip():
            return
        self.console.print("[dim italic]─── FINAL SYNTHESIS ───[/dim italic]")
...
class LangChainEngine:
    ...
    def _get_system_prompt(self) -> str:
        ...
        [DOCUMENT / PDF Guide]
        ONLY create document/pdf when asked - IMPORTANT.
    ...
    def node_complete(self, state: AgentState) -> Dict[str, Any]:
        ...
        """
        with self.ui.node_context("FINALIZE") as live:
