    ...
    def generate_response_streamed(self, history: List[Dict[str, str]], user_input: str, ui: Any, status_callback=None) -> 'AgentResponse':
        ...
        try:
            ...
            def attempt_parse(raw_text: str) -> AgentResponse:
                ...
                # 2. Handle the "Invalid Control Character" (unescaped newlines) issue
                # We find content inside quotes and escape actual newlines within them
                def escape_newlines(m):
                    return m.group(0).replace('\n', '\\n').replace('\r', '\\r')
                json_str = re.sub(r'"(?:\\.|[^"\\])*"', escape_newlines, json_str)

                # 3. Load into a dictionary to perform structural repairs
                try:
                    data = json.loads(json_str)
                except json.JSONDecodeError:
                    # Last ditch effort: try removing trailing commas before parsing
                    json_str = re.sub(r',\s*([\]}])', r'\1', json_str)
                    data = json.loads(json_str)

                # 4. THE FIX: Structural Repair (List -> String conversion)
                # If LLM sends trajectory as a list, join it into a single string
                if "trajectory" in data and isinstance(data["trajectory"], list):
                    data["trajectory"] = "\n".join([str(item) for item in data["trajectory"]])
                
                # Ensure synthesis is a string (sometimes LLMs send null or numbers)
                if "synthesis" in data and not isinstance(data["synthesis"], str):
                    data["synthesis"] = str(data["synthesis"])

                # 5. Return validated model via dictionary unpacking
                return AgentResponse(**data)

            try:
                # Try standard parsing first for performance
                return self.parser.parse(full_content)
            except Exception:
                # Fallback to the robust repair logic
                return attempt_parse(full_content)

        except Exception as e:
            raise ValueError(f"Looks like the AI is a little dumb, Don't quit | Error: {str(e)}")
