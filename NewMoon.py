        ...
        try:
            for chunk in self.llm.stream(formatted_prompt):
                ...
                if content:
                    ...
                    # Real-time status extraction for the UI spinner
                    if status_callback:
            ...
            def robust_parse(raw_text: str) -> AgentResponse:
                # 1. Extract the JSON block using regex (handles markdown blocks or conversational filler)
                ...
                # Even with valid JSON, LLMs often violate the schema types.
                
                # Fix trajectory: if it's a list, join into a string
                ...
                # Fix synthesis: ensure it's a string (not None or number)
                ...
                # Fix action: ensure it's either a dict or None (not a string/null)
            ...
            # Attempt the robust parse
            return robust_parse(full_content)

        except Exception as e:
            # Provide context in the error for easier debugging during development
...
class LangChainEngine:
    ...
    def run_mission(self, mission: str, previous_state: Optional[Dict[str, Any]] = None):
        ...
        if previous_state:
            self.ui.log_system_status(" Resuming")
