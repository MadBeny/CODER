...
CODER_THEME = Theme({
    ...
    "accent": "white",
    "border": "grey37",
    "markdown.text": "grey93 italic",
    "markdown.h1": "white bold",
    "markdown.h2": "grey89 bold",
    "markdown.h3": "grey85 bold",
    "markdown.h4": "grey74 bold",
    "markdown.h5": "grey62",
    "markdown.h6": "grey50",
    "markdown.strong": "white bold",
    "markdown.em": "grey70 italic",
    "markdown.code": "grey85",
    "markdown.link": "grey85",
    "markdown.blockquote": "grey42",
    "markdown.hr": "grey30",
    "markdown.table.header": "white bold",
    "markdown.table.cell": "grey82"
})
...
class AgentResponse(BaseModel):
    trajectory: str = Field(description="roadmap, should always be a string/str")
    status_summary: str = Field(description="Status update")
    synthesis: str = Field(description="Reasoning/Thought process")
    action: Optional[ToolCall] = Field(default=None, description="The tool to call, or null if finishing")
    completion_summary: Optional[str] = Field(default=None, description="The answer")
...
class RichUI:
    ...
    def node_context(self, name: str, is_error: bool = False):
        ...
        tag_style = "white on red" if is_error else "white on grey23"
        ...
        with Live(Text("", style="subtle"), console=self.console, refresh_per_second=60) as live:
            def animate():
                while state.running:
                    if state.message:
            ...
            try:
                ...
                if not is_error:
                    # Only render the name tag with white on green style, no separator or emoji
                    success_tag = Text(f" {name.upper()} ", style="white on green")
                    live.update(success_tag)
                else:
                    fail_text = Text()
    ...
    def log_system_status(self, msg: str): self.console.print(f"[dim italic]{msg}[/dim italic]")
...
class LLMInterface:
    def __init__(self, model_name: str, base_system_prompt: str):
        self.llm = ChatOllama(model=model_name, temperature=1.2)
        self.base_system_prompt = base_system_prompt
        # We keep the parser for instruction generation, but we won't rely on its 
        # strictness for actual parsing since it fails easily on local models.
...
class LLMInterface:
    ...
    def generate_response_streamed(self, history: List[Dict[str, str]], user_input: str, ui: Any, status_callback=None) -> 'AgentResponse':
        ...
        try:
            ...
            def robust_parse(raw_text: str) -> AgentResponse:
                ...
                # <3 <3 <3 <3 <3 <3 <3
                # 2. Use json-repair to handle syntax errors
                try:
                    data = json_repair.loads(json_str)
                except Exception as e:
                    raise ValueError(f"json-repair failed to recover the JSON structure: {e}")

                if not isinstance(data, dict):
                    raise ValueError(f"Parsed JSON is a {type(data)}, expected a dictionary.")

                # 3. TYPE COERCION (The "Safety Net")
                
                # Trajectory: List -> String
                if "trajectory" in data and isinstance(data["trajectory"], list):
                    data["trajectory"] = "\n".join([str(item) for item in data["trajectory"]])
                elif "trajectory" not in data:
                    data["trajectory"] = ""

                # Synthesis: Ensure String
                if "synthesis" in data and not isinstance(data["synthesis"], str):
                    data["synthesis"] = str(data.get("synthesis", ""))
                elif "synthesis" not in data:
                    data["synthesis"] = ""

                # Status Summary: Ensure String
                if "status_summary" in data and not isinstance(data["status_summary"], str):
                    data["status_summary"] = str(data.get("status_summary", ""))
                elif "status_summary" not in data:
                    data["status_summary"] = ""

                # --- NEW FIX: Completion Summary Coercion ---
                # If the LLM sends null, it's fine. 
                # If it sends anything else that isn't a string (like an empty dict or number), cast to str.
                if "completion_summary" in data and data["completion_summary"] is not None:
                    if not isinstance(data["completion_summary"], str):
                        data["completion_summary"] = str(data["completion_summary"])

                # Action: Ensure it's either a dict or None
                ...
                # 4. Final Validation via Pydantic
                return AgentResponse(**data)

            return robust_parse(full_content)

        except Exception as e:
            raise ValueError(f"AI Response Parsing Error | {str(e)}")
...
class LangChainEngine:
    ...
    def _get_system_prompt(self) -> str:
        ...
        return """
        [IDENTITY: MOON]
        Use polished, formal language and maintain an aura of calm, unwavering respect.
        Be helpful, articulate, and unfailingly polite.
        Maintain a calm, steady presence; respond to all inquiries with graciousness and effortless courtesy.

        [TOOLKIT]
        - list_files(directory='.') Scans directory contents
        - get_tree(directory='.', max_depth=3) Visualizes filesystem structure
        - search_files(pattern, directory='.') Finds files via glob pattern
        - grep_search(pattern, file_path, context_lines=3) Searches text within a file
        - python_executor(code) Runs arbitrary Python code
        - run_file(filename) Executes a specific .py file
        - shell_execute(command) Low-level system commands
        - manage_process(action, pid=None) Lists or kills processes
        - get_system_info() Returns host environment snapshot
        - get_file_stats(filename) Detailed file metadata
        - scientific_compute(expression) High-precision math/numpy
        - read_file(filename, start_line=1, end_line=None) Reads specific lines of a file
        - read_pdf (filename) Extracts text from PDF
        - write_file(filename, content, mode='overwrite') Overwrites/creates a file
        - append_to_file(filename, content) Adds content to end of file
        - replace_in_file(filename, search_pattern, replacement) String or Regex replacement
        - get_file_context(filename, line_start, line_end) Reads lines for context
        - replace_lines(filename, start_line, end_line, content) Surgical line replacement
        - patch_regex(filename, pattern, replacement) Pattern-based edits
    ...
    def node_error(self, state: AgentState) -> Dict[str, Any]:
        with self.ui.node_context("ERROR", is_error=True) as live:
            ...
            self.ui.log_error(f"Don't worry, I'll fix it: {err_msg}")
